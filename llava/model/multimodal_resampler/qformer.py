...
class Qformer(nn.Module):
    def __init__(self, model_args, vision_tower=None):
        super().__init__()
        ...
        try:
            self.q_former, self.query_tokens, self.ln_vision = self.build_Qformer(hidden_size, self.depth, self.num_latents)

            if self.pretrained is not None:
                try:
                    pretrained_dict = torch.load(self.pretrained, map_location="cpu")["model"]
                except:
                    pretrained_dict = torch.load(self.pretrained, map_location="cpu")["state_dict"]
                pretrained_dict = {k: v for k, v in pretrained_dict.items() if not k.startswith("t5_proj")}
                self.load_state_dict(pretrained_dict)
            self.Qformer = self.q_former
        except:
            self.Qformer, self.query_tokens, self.ln_vision = self.build_Qformer(hidden_size, self.depth, self.num_latents)
...
if __name__ == "__main__":
    ...
    ckpt = torch.load("/mnt/bn/chenhaobo-va-data/wangmingze/Research_code/pretrain_ckpt/BLIP2/minigpt-4_qformer_20230615-1dfa889c.pth")
    class MODEL_ARGS():
        def __init__(self) -> None:
            self.mm_qformer_depth = 2
            self.mm_qformer_latents = 32
            self.mm_qformer_pretrained = None
            self.mm_qformer_pretrained = "/mnt/bn/chenhaobo-va-data/wangmingze/Research_code/pretrain_ckpt/BLIP2/minigpt-4_qformer_20230615-1dfa889c_resized1152.pth"
    ...
    query_output = model(image_features)
