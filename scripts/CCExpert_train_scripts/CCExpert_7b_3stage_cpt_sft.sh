...
# >>>>>>>>>>>>>>>>>>>>>>>>>> CPT Stage1 Run <<<<<<<<<<<<<<<<<<<<<<<<<<<
CPT_STAGE1_RUN_NAME="CCExpert_7b_stage1_cpt"
...
# >>>>>>>>>>>>>>>>>>>>>>>>>> CPT Stage2 Run <<<<<<<<<<<<<<<<<<<<<<<<<<<
CPT_STAGE2_RUN_NAME="CCExpert_7b_stage2_cpt"
...
# >>>>>>>>>>>>>>>>>>>>>>>>>> LEVIR-CC SFT Run <<<<<<<<<<<<<<<<<<<<<<<<<<<
SFT_RUN_NAME="CCExpert_7b_sft_levircc"
...
ACCELERATE_CPU_AFFINITY=1 torchrun --nproc_per_node="${NUM_GPUS}" --nnodes="${NNODES}" --node_rank="${RANK}" --master_addr="${ADDR}" --master_port="${PORT}" \
    ...
    # --data_path "/mnt/bn/chenhaobo-va-data/wangmingze/Research_code/LLaVA-NeXT/scripts/CC_project/data/cptdata.yaml" \